...
# Correlation
```{r}
colnames(DATA)<- c("Date","EVI","Precipitation","Evapotranspiration","TempMin","TempMax","Humidity","Drought")
plot(corr_coef(DATA))
```
# Select a set of predictors with minimal multicollinearity
```{r}
non_collinear_vars(DATA,TempMin,TempMax,Evapotranspiration,Precipitation,Humidity,Drought,max_vif =3)
```
# Removed correlated variable
```{r}
DATA <- DATA%>%
dplyr::select(Date,Drought, TempMin, Precipitation, TempMax, Evapotranspiration,EVI)
...
# Linear Model
```{r}
# summary(lm(mean_evi~Precipitation+Evapotranspiration+MiniTemperature+MaxTemperature+Drought,data = DATA))
...
adf.none  <- list(
    ...
    Evapotranspiration = ur.df(DATA$Evapotranspiration, type='none', selectlags = c("BIC")),
...
adf.drift  <- list(
    ...
    Evapotranspiration = ur.df(DATA$Evapotranspiration, type='drift', selectlags = c("BIC")),
...
adf.trend  <- list(
    ...
    Evapotranspiration = ur.df(DATA$Evapotranspiration, type='trend', selectlags = c("BIC")),
...
df.adf.none  <- list(
    ...
    Evapotranspiration = ur.df(df$Evapotranspiration, type='none', selectlags = c("BIC")),
...
df.adf.drift  <- list(
    ...
    Evapotranspiration = ur.df(df$Evapotranspiration, type='drift', selectlags = c("BIC")),
...
df.adf.trend  <- list(
    ...
    Evapotranspiration = ur.df(df$Evapotranspiration, type='trend', selectlags = c("BIC")),
...
```
The ADF result for LRM variable from the above R code is generated as follows and our focus is on the yellow rectangular area which shows the ADF test result.
...
adf.trend$EVI
```
Interpretation of ADF test follow the general-to-specific approach. As such, three regression models are applied sequentially. For interpretation, let’s take LRY variable for example by using the following R code.
```{r}
#========================================================
...
print("Level Variable with Trend")
cbind(t(adf.trend$Evapotranspiration@teststat), adf.trend$Evapotranspiration@cval)
...
Evapotranspiration <- ts(data = DATA$Evapotranspiration, start = c(2000, 1), end = c(2022, 01), frequency = 12)
...
TimeSeries <- cbind(EVI,Precipitation,Evapotranspiration,TempMin,TempMax,Drought)
```
...
grangertest(EVI~Evapotranspiration, order = 12)
grangertest(EVI~Drought, order = 12)
```

```{r}
plot(fevd(Model1,n.ahead = 12))
```

```{r}
data <- DATA%>%select(Date,Drought, TempMin, Precipitation, TempMax, Evapotranspiration,EVI)
...
f_mc <- as.formula("EVI~Drought+ TempMin+ Precipitation+ TempMax+ Evapotranspiration")
...
models <- train %>% 
  model(ARIMA = ARIMA(f_mc, stepwise = FALSE),
        ...
        ETS = ETS(EVI),
        TSLM = TSLM(EVI~Drought+ TempMin+ Precipitation+ TempMax+ Evapotranspiration+
                      trend() + season())) %>% 
