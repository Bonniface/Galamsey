...
`{# {r,include=FALSE} # #This will take about 30 minutes # if(readline(prompt = "Hit enter to proceed or type 'no' to download the data from G-Drive. ") == "no"){ # googledrive::drive_download(file = #                               googledrive::as_id("https://drive.google.com/drive/folders/1ZnCpYz38ezSU1XG7ixJ2sPg_DX7bO07J?usp=sharing"),overwrite = T) #   #https://drive.google.com/drive/folders/1ZnCpYz38ezSU1XG7ixJ2sPg_DX7bO07J?usp=sharing # evi.df <- read.csv("rgee_file_2d44527a3b0e_2022_07_28_15_40_52.csv") # evi.df <- evi.df[,3:ncol(evi.df)] # colnames(evi.df) <- c('hex_id', stringr::str_replace_all(substr(colnames(evi.df[, 2:ncol(evi.df)]), 2, 11), "_", "-")) #Convert dates to unambiguous format # } else { # #This will take about 30 minutes # paste0(system.time(expr = aoi.evi <- ee_extract(x = modis.evi, y = hex["hex_id"], sf = FALSE, scale = 250, fun = ee$Reducer$mean(), via = "drive", quiet = T))/60, " Minutes Elapsed. ") # evi.df <- as.data.frame(aoi.evi) # colnames(evi.df) <- c('hex_id', stringr::str_replace_all(substr(colnames(evi.df[, 2:ncol(evi.df)]), 2, 11), "_", "-")) # write.csv(x = evi.df, file = "~/rgee_file_2d44527a3b0e_2022_07_28_15_40_52.csv")}`
...
`{# {r,warning=FALSE,message=FALSE} # adf.test(Tt)`
...
# tdx.ns <- data.frame(time = c(1:length(Time_Series)), trend = tdx - tdx.dcp$time.series[,1])
